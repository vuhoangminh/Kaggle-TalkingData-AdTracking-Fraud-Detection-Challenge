...
def lgb_modelfit_nocv(params, dtrain, dvalid, predictors, target='target', objective='binary', metrics='auc',
    ...
    bst1 = lgb.train(lgb_params, 
                     ...
                     verbose_eval=10, 
                     learning_rates=lambda iter: 0.001 * (0.995 ** iter),
                     feval=feval)
...
def train(iSplit):
    ...
    predictors = ['app','device','os', 'channel', 'ipcount', 'qty', 'ip_app_count', 'ip_app_os_count']
    ...
    params = {
        ...
        'bagging_fraction': 0.4, # randomly select part of data without resampling
        'bagging_freq': 10, # frequency for bagging
    ...
    print("writing...")
    save_name = 'lightgbm_rf_v1_' + str(iSplit) 
    sub.to_csv(save_name + '.csv',index=False,float_format='%.2f')
    print("done...")
    del test_df
    gc.collect()

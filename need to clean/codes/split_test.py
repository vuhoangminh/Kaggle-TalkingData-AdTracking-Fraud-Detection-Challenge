...
def load_write(iSplit):
    ...
    train_df = pd.read_csv(path+"test.csv", dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    ...
    train_df = train_df.drop(['ip', 'click_time', 'wday'], axis=1)
    
    print(train_df.head())


    print("test size: ", len(train_df))

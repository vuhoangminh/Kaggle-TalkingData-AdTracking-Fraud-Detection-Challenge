...
# nrows = 75000000
output_filename = 'lightgbm_single.csv'
...
def prep_data( df ):
    ...
    print("extract hour, day...")    
    df['hour'] = pd.to_datetime(df.click_time).dt.hour.astype('uint8')
    df['day'] = pd.to_datetime(df.click_time).dt.day.astype('uint8')
    df.drop(['click_time'], axis=1, inplace=True)
    gc.collect()
    
    print("create in test hh...")
    df['in_test_hh'] = (   3 
...
train_df.to_pickle(path+'train_new')
gc.collect()
...
test_df.to_pickle(path+'test_new')
gc.collect()

...
which_dataset = 0      
print('load train....')
train_set_name = 'train_' + str(which_dataset)
...
print('load test....')
test_set_name = 'test_' + str(which_dataset)
print(test_set_name)
...
y_valid = valid_df['is_attributed'].values
...
valid_df = get_keras_data(valid_df)
...
model = Model(inputs=[in_app,in_ch,in_dev,in_os,in_h,
                in_d,in_wd,in_qty,in_c1,in_c2], outputs=outp)
...
model.compile(loss='binary_crossentropy',optimizer=optimizer_adam,metrics=[jacek_auc, 'accuracy'])
...
filepath="model_best_auc_" + str(which_dataset) + ".hdf5"
savepath = "model_best_auc_" + str(which_dataset) + ".h5"
csvpath = "dl_auc_" + str(which_dataset) + ".csv"
logpath = "log_dl_auc_" + str(which_dataset) + ".csv"
print ("file path to save best weights:", filepath)
print ("file path to save best model:", savepath)
...
model.fit(train_df, y_train, batch_size=batch_size, epochs=epochs, class_weight=class_weight, 
                shuffle=True, verbose=1, validation_data = (valid_df, y_valid),
